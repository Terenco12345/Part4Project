...
public class Board : MonoBehaviour
{
     ...
     * Row 1: Col 5->8, 3 tiles
     * Row 2: Col 4->8, 4 tiles
     * Row 3: Col 3->8, 5 tiles
     * Row 4: Col 3->7, 4 tiles
     * Row 5: Col 3->6, 3 tiles
     */
    ...
    public void RandomizeTileTypes()
    {
        ...
        {
            tileObjects[i].GetComponent<Tile>().resourceType = resources[i];
        }
    }
    ...
    public void SetupChanceTokens()
    {
        ...
        for(int i = 0; i < tokenOrderCol.Length; i++)
        {
            ...
            if(!(tile.GetComponent<Tile>().resourceType == ResourceType.Desert || tile.GetComponent<Tile>().resourceType == ResourceType.None))
            {
                tile.GetComponent<Tile>().chanceTokenValue = tokenValues[tokenIndex];
                tokenIndex++;
            }
        }
    }
    ...
    public void MoveRobberToDesert()
    {
        ...
        for(int i = 0; i < tileObjects.Length; i++)
        {
            ...
            if (tile.GetComponent<Tile>().resourceType == ResourceType.Desert)
            {
                robber.GetComponent<Robber>().tile = tile;
            }
        }
    }
    ...
    {
        robber.GetComponent<Robber>().tile = boardGrid.GetFace(col, row);
    }

    void LateUpdate()
    {
        if(robber.GetComponent<Robber>().tile != null)
        {
            robber.transform.localPosition = robber.GetComponent<Robber>().tile.transform.localPosition;
        }
    }
}
